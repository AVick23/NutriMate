...
from analytics.core import calculate_trend_slope
...
# ============================================================
# ФОРМАТИРОВАНИЕ ПАТТЕРНОВ (ОБНОВЛЕНО: добавлен confidence_score)
# ============================================================
def format_patterns(patterns: List[Any], max_count: int = 5) -> str:
    ...
    for i, pattern in enumerate(patterns[:max_count], 1):
        ...
        # 🎯 Используем новый confidence_score (0-100)
        confidence = getattr(pattern, 'confidence_score', 0)
        conf_text = f" (уверенность: {confidence}%)" if confidence > 0 else ""
        ...
        lines.append(
            f"{i}. {emoji} <b>{pattern.effect_text}</b>{lag_text}{conf_text}\n"
            f"   Связь: r={pattern.correlation_r:.2f}, подтверждений: {pattern.sample_size}"
        )
...
# ============================================================
# ФОРМАТИРОВАНИЕ СОСТОЯНИЙ (ОБНОВЛЕНО: добавлен risk_score)
# ============================================================
def format_states(states: List[Any]) -> str:
    ...
    for state in active_states:
        ...
        # 🎯 Новый risk_score (например, 4/5)
        risk_score = getattr(state, 'risk_score', 0)
        risk_text = f" (уверенность: {risk_score}/5)" if risk_score > 0 else ""
        
        lines.append(f"{state.emoji} <b>{state_name_ru(state.state_type)}</b> {severity_emoji}{risk_text}")
...
# ============================================================
# ФОРМАТИРОВАНИЕ ПРОГНОЗА (ОБНОВЛЕНО: линейная регрессия)
# ============================================================
def format_forecast(aggregates: List[Any], profile: Dict[str, Any]) -> str:
    """Форматирует прогноз достижения цели с использованием линейной регрессии."""
    if len(aggregates) < 7:
        return (
            "🔮 <b>Прогноз</b>\n\n"
    ...
    # 🎯 УМНЫЙ РАСЧЁТ: используем наклон тренда, а не просто last - first
    ...
    if target_weight:
        ...
        if slope != 0 and remaining != 0:
            weeks_to_goal = remaining / (slope * 7)
            days_to_goal = int(abs(weeks_to_goal) * 7)
            
            if days_to_goal > 0:
                import datetime
                target_date = datetime.date.today() + datetime.timedelta(days=days_to_goal)
                direction = "достигнута" if (slope < 0 and remaining < 0) or (slope > 0 and remaining > 0) else "изменится"
                
                lines.append(f"\n📅 При текущем тренде ({weekly_change:+.1f} кг/нед):")
                lines.append(f"   Цель будет {direction} через <b>{days_to_goal} дней</b>")
                lines.append(f"   Примерная дата: <b>{target_date.strftime('%d.%m.%Y')}</b>")
            else:
                lines.append("\n✨ <b>Цель уже достигнута!</b>")
        else:
            lines.append("\n⏸️ <i>Тренд слишком плоский для точного прогноза</i>")
    else:
    ...
    if weekly_change < -1.0:
        ...
        lines.append("\n✅ Темп снижения веса в норме")
    elif -0.3 < weekly_change < 0.3:
...
# ============================================================
# ФОРМАТИРОВАНИЕ МОДИФИКАТОРОВ TDEE
# ============================================================
