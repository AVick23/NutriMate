"""
Обработчики для замеров тела.
🎯 Обновлено: MeasurementsRepository импортируется из db.repositories,
   удалена заглушка "Мои цели", добавлены динамические быстрые значения
   и возможность удаления замеров из истории.
"""
import logging
...
class MeasurementsHandlers:
    ...
    async def handle_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        # 🎯 УДАЛЕНО: обработка CALLBACK_MEASUREMENTS_GOALS (заглушка убрана)
    ...
    async def handle_type_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if data.startswith(CALLBACK_TYPE_PREFIX):
            ...
            # 🎯 Получаем последний замер для динамических кнопок
            user = update.effective_user
            user_id = await self.user_repo.get_user_id(user.id)
            last = await self.measurements_repo.get_last_measurement(user_id, type_id)
            ...
            await query.edit_message_text(
                text,
                reply_markup=get_value_keyboard(type_id, last_value),  # 🎯 передаём last_value
                parse_mode="HTML"
            )
    ...
    async def _save_measurement(
        ...
        value: float, is_callback: bool = True
    ) -> int:
        ...
        # Добавляем оценку риска по талии (ВОЗ)
        if info["name"] == "waist":
    ...
    async def _show_history(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE, type_id: int
    ) -> None:
        """Показывает историю замеров выбранного типа (с кнопками удаления)."""
        query = update.callback_query
        user = update.effective_user
        user_id = await self.user_repo.get_user_id(user.id)
        ...
        await query.edit_message_text(
            text,
            reply_markup=get_history_list_keyboard(history, type_id, info["unit"]),
            parse_mode="HTML"
        )
    ...
    # 🎯 УДАЛЕНИЕ ЗАМЕРА
    # ================================================================
    async def handle_delete_measurement(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """Удаляет замер по ID и возвращает к выбору типа истории."""
        query = update.callback_query
        await query.answer("🗑 Удаляю...")

        try:
            record_id = int(query.data.replace(CALLBACK_DELETE_PREFIX, ""))
            await self.measurements_repo.delete_measurement(record_id)
        except (ValueError, Exception) as e:
            logger.error(f"Ошибка удаления замера: {e}")
...
def get_measurements_handler(db: Database) -> ConversationHandler:
    ...
    # 🎯 УДАЛЕНО: asyncio.create_task(handlers.measurements_repo.init_tables())
    # Таблицы создаются централизованно в db/database.py
    ...
    return ConversationHandler(
        ...
        states={
            ...
            STATE_HISTORY_TYPE: [
                CallbackQueryHandler(handlers.handle_history_type, pattern="^measurements_history_type_"),
                # 🎯 НОВОЕ: обработка удаления замеров
                CallbackQueryHandler(
                    handlers.handle_delete_measurement,
                    pattern=f"^{CALLBACK_DELETE_PREFIX}"
                ),
