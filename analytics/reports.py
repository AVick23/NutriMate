...
from .core import DailyAggregate
...
class WeeklyReportGenerator:
    ...
    async def generate_report(self, user_id: int, profile: dict, end_date: date = None) -> str:
        if end_date is None:
        ...
        avg_kcal = self._safe_avg([a.nutrition.total_kcal for a in aggregates])
        avg_protein = self._safe_avg([a.nutrition.total_protein_g for a in aggregates])
        avg_steps = self._safe_avg([a.activity.steps for a in aggregates if a.activity.steps])
        avg_sleep = self._safe_avg([a.sleep.hours for a in aggregates if a.sleep.hours])
        if avg_kcal: report.append(f"🔥 Калории: {avg_kcal:.0f} ккал/день")
        if avg_protein: report.append(f"🍗 Белок: {avg_protein:.0f} г/день")
        if avg_steps: report.append(f"👣 Шаги: {avg_steps:.0f} шагов/день")
        if avg_sleep: report.append(f"😴 Сон: {avg_sleep:.1f} ч/день")
        ...
        if states:
            ...
            for s in states:
                if s.detected:
                    report.append(f"{s.emoji} <b>{self._state_name(s.state_type)}</b>")
    ...
    def _state_name(self, state_type: str) -> str:
        from .intelligence import STATE_NAMES
